...
function showPadAds() {
    ...
    for (var i = 0; i < elms.length; i++)
        elms[i].innerHTML = '<iframe src="https://phonglan123.github.io/pad/" style="position: absolute; top: -1000px; left: -1000px"></iframe><div style="border: 1px solid #ccc; height: 60px; width: 100%; user-select: none; cursor: pointer; font: 15px Arial, sans-serif; margin: auto; background: white; text-align: left;"> <div style="border: 1px solid #ccc; background: white; position: absolute; margin-top: 38px; width: 50px; text-align: center; margin-left: 110px; font-size: 14px;" onclick="window.open(' + "'" + adsRegisterLink + "'" + '); showPadAds();">' + adsAlert + '</div> <div onclick="window.open(' + "'https://phonglan123.github.io/pad/go.html?target=" + encodeURIComponent(link) + "'" + '); showPadAds();"> <img src="' + imageURL + '" style="margin: 5px; height: 50px; width: 100px"/> <div style="width: calc(100% - 115px); margin-top: -58px; margin-left: 110px; text-align: justify; height: 37px; overflow: hidden;"><b>' + title + '</b></div><div style="width: calc(100% - 170px); overflow: hidden; margin-left: 165px; white-space: nowrap;">' + new URL(link).hostname + '</div></div></div>';
}
